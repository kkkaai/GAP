...
#             )
#         return list(zip(image_paths, mask_paths, strict=True))
...
#         prompt_images = [self._rgb_to_pil(load_image(image_path)) for image_path, _ in support_pairs]
...
DEFAULT_SUPPORT_ROOT = Path(__file__).resolve().parents[1] / "src_pic"
DEFAULT_SUPPORT_MASK_ROOT = DEFAULT_SUPPORT_ROOT / "mask"
DEFAULT_SUPPORT_NAMES = ("1", "2", "3", "4")
...
class Phase1Mask:
    ...
    @staticmethod
    def _find_default_support_image(stem: str) -> Path:
        for extension in SUPPORTED_IMAGE_EXTENSIONS:
            candidate = DEFAULT_SUPPORT_ROOT / f"{stem}{extension}"
            if candidate.exists():
                return candidate
        raise FileNotFoundError(f"Could not find support image {stem}.* in {DEFAULT_SUPPORT_ROOT}.")

    def _default_support_pairs(self) -> list[tuple[str, str]]:
        pairs: list[tuple[str, str]] = []
        for name in DEFAULT_SUPPORT_NAMES:
            image_path = self._find_default_support_image(name)
            mask_path = DEFAULT_SUPPORT_MASK_ROOT / f"{name}.png"
            if not mask_path.exists():
                raise FileNotFoundError(f"Could not find support mask {mask_path}.")
            pairs.append((str(image_path), str(mask_path)))
        return pairs

    def _resolve_support_pairs(self) -> list[tuple[str, str]]:
        image_paths = self._clean_paths(self.config.support_image_paths)
        mask_paths = self._clean_paths(self.config.support_mask_paths)
        if self.config.support_image_path:
            image_paths.append(self.config.support_image_path)
        if self.config.support_mask_path:
            mask_paths.append(self.config.support_mask_path)
        if not image_paths and not mask_paths:
            return self._default_support_pairs()
        if len(image_paths) != len(mask_paths):
            raise ValueError(
                "SegGPT support image and mask counts must match: "
                f"{len(image_paths)} image(s), {len(mask_paths)} mask(s)."
