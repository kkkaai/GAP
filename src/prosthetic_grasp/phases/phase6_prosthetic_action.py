...
class Phase6ProstheticActionConfig:
    """mano2prosthetic position-only fingertip retargeting baseline.
    ...
    ring/little order. This baseline keeps the robot wrist/base fixed and
    optimizes only robot joint action from URDF/MJCF kinematics.

    To add another hand under ``hand/``, set ``robot_profile = "custom"`` and
    provide ``robot_urdf_path`` or ``robot_xml_path`` plus ``fingertip_links``.
    Optionally provide ``joint_names`` to restrict or reorder optimized joints.
    """
...
class Phase6ProstheticAction:
    """Run the mano2prosthetic position-only fingertip baseline for pipeline phase6."""
    ...
    def run(self, phase5_result: Phase5ManoResult | Any) -> Phase6ProstheticActionResult:
        ...
        return Phase6ProstheticActionResult(
            status="ok",
            message=(
                "Computed mano2prosthetic position-only fingertip retargeting action."
            ),
            ...
            metadata={
                ...
                "baseline": "mano2prosthetic.position_only",
