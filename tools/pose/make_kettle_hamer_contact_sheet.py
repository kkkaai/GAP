...
def main() -> None:
    ...
    for sample in SAMPLES:
        for ldir in sorted((ROOT / sample).glob("lollipop_*")):
            p = ldir / "pose_hamer" / "comparison.png"
            if p.exists():
                paths.append(p)
    imgs = []
    for p in paths:
        img = imageio.imread(p)[..., :3]
        img = cv2.resize(img, (640, 240), interpolation=cv2.INTER_AREA)
        label = f"{p.parents[1].name}/{p.parents[0].name}"
        cv2.putText(img, label, (10, 26), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 3, cv2.LINE_AA)
        cv2.putText(img, label, (10, 26), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (20, 20, 20), 1, cv2.LINE_AA)
        imgs.append(img)
    rows = []
    ...
    out = ROOT / "kettle_hamer_contact_sheet.png"
